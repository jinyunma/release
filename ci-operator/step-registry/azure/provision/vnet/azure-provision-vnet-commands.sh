...
if [[ "${IPSTACK}" == "dualstack" ]]; then
    ...
    readarray -t compute_ipv6_subnets < <(get_next_subnet ${worker_ipv6} ${AZURE_COMPUTE_SUBNETS_NUMBER})
fi
...
readarray -t compute_ipv4_subnets < <(get_next_subnet ${AZURE_COMPUTE_SUBNET_PREFIX} ${AZURE_COMPUTE_SUBNETS_NUMBER})
