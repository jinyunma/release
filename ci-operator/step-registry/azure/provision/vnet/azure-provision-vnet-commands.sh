...
if [[ "${IPSTACK}" == "dualstack" ]]; then
    ...
    readarray -t compute_ipv6_subnets < <(get_next_subnet ${worker_ipv6} $((AZURE_COMPUTE_SUBNETS_NUMBER - 1)))
...
readarray -t compute_ipv4_subnets < <(get_next_subnet ${AZURE_COMPUTE_SUBNET_PREFIX} $((AZURE_COMPUTE_SUBNETS_NUMBER - 1)))
compute_ipv4_subnets=("${AZURE_COMPUTE_SUBNET_PREFIX}" "${compute_ipv4_subnets[@]}")
for i in $(seq 0 $((AZURE_COMPUTE_SUBNETS_NUMBER - 1))); do
...
if [[ "${AZURE_SUBNETS_ENABLED}" == "yes" ]]; then
